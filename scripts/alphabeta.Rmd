...
```{r}
seqtab.nochim <- readRDS(paste0(path, "analysis/CLEAN_merged_seqtabNochim_20230718.rds"))
taxa <- readRDS(paste0(path, "analysis/CLEAN_taxonomy_merged_20230718.rds"))

sam <- read.table(paste0(path, "metadata/metadata_rapaMem.csv"), header=T, row.names=1, check.names=F, sep=";")
count_tab <- read.table(paste0(path, "analysis/CLEAN_ASVs_counts_merged_20230718.tsv"), 
...
p +  scale_color_manual(values = c("grey", "green", "dodgerblue", "red")) + facet_wrap(~Treatment)
dev.off()
...
plot_ordination(ps, uf_ord, color="Treatment") + theme(aspect.ratio=1) +  scale_color_manual(values = c("grey", "green", "dodgerblue", "red"))
dev.off()
...
pdf(paste0(path,"analysis/plots/alpha_beta_diversity/unifraq_unweighted_PCoA_Tech_", new_day, ".pdf"))
plot_ordination(ps, uf_ord, color="Tech")
dev.off()
...
plot_ordination(ps, wuf_ord, color="Treatment") + theme(aspect.ratio=1) +  scale_color_manual(values = c("grey", "green", "dodgerblue", "red"))
dev.off()
...
pdf(paste0(path,"analysis/plots/alpha_beta_diversity/unifraq_weighted_PCoA_Tech_", new_day, ".pdf"))
plot_ordination(ps, wuf_ord, color="Tech") + theme(aspect.ratio=1)
dev.off()
```
...
plot_ordination(ps, bray_ord, color="Treatment") + theme(aspect.ratio=1, axis.text=element_text(size=14), axis.title=element_text(size=16), plot.title = element_text(size=18)) + geom_point(size = 1) +  scale_color_manual(values = c("grey", "green", "dodgerblue", "red")) + labs(title = "PCoA/Bray-Curtis")
dev.off()
